		...
		<?php if ( get_post_meta( $post->ID, 'ecpt_majors', true ) || get_post_meta( $post->ID, 'ecpt_minors', true ) ) : ?>
			<div class="mt-4 pr-4">
				...
				<?php if ( get_post_meta( $post->ID, 'ecpt_majors', true ) ) : ?>
					<span class="inline-block text-primary bg-blue-light font-heavy font-bold text-lg px-2 mx-1">Major</span>
				<?php endif; ?>
				<?php if ( get_post_meta( $post->ID, 'ecpt_minors', true ) ) : ?>
					<span class="inline-block text-primary bg-blue-light font-heavy font-bold text-lg px-2 mx-1">Minor</span>
				<?php endif; ?>
			</div>
		<?php endif; ?>
		<?php if ( get_post_meta( $post->ID, 'ecpt_degreesoffered', true ) ) : ?>
			<div class="mt-4 pr-4">
				<span>Degrees Offered:</span> <span class="inline-block text-primary bg-blue-light font-heavy font-bold text-lg px-2 mx-1"><?php echo esc_html( get_post_meta( $post->ID, 'ecpt_degreesoffered', true ) ); ?></span>
			</div>
		<?php endif; ?>
	</div>
